...
def test_column_boundaries_and_bucketing():
    ...
    cols = its.bucket_columns(
        [word("z", 31, 0), word("x", 9, 0), word("y1", 18, 0), word("y2", 22, 0)], boundaries
    )
...
def usa_pages():
    ...
    girls = [word("36.29", 10, 90), word("33.09", 20, 90), word(":29.59", 30, 90),
             # AA (x=40) intentionally blank: no standard published
             word("bad", 50, 90), word("25.00*", 60, 90)]
    evt = [word("50", 100, 90), word("FR", 108, 90), word("SCY", 116, 90)]
    boys = [word(t, x, 90) for t, x in zip(
        ["24.00", "25.00", "26.00", "27.00", "28.00", "1:02.00"], BOYS_X)]
    relay = [word("200", 100, 110), word("FR-R", 108, 110), word("SCY", 116, 110), word("1:50.00", 10, 110)]

    return [
        FakePage(header + before_header + age_row + girls + evt + boys + relay,
                 text="2024-2028 Motivational Standards", page_number=1),
